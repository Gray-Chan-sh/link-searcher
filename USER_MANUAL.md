...
3. 结果会**混合**两类匹配：关键词命中的 + 语义相近的，按**可调权重融合**——设置页 AI 区「检索策略」滑杆控制偏向（默认语义 30%/关键词 70%，拖向语义找"意思相近"，拖向关键词找精确短语）
...
> 修改配置（排除规则/包含类型/OCR 语言/递归）后保存，会自动对该目录触发一次**增量扫描**，变更立即生效。
...
每次扫描（启动/手动/重建/添加目录）会生成独立日志文件 `data/logs/scan-{时间戳}.log`。「日志查看」页可查看历史会话日志列表与内容。

日志页支持**关键字过滤**（顶部输入框，按内容实时筛选）与**类型标签**（全部/索引/OCR/扫描/搜索/错误）。

### 5.2b2 任务简报

长任务（验证索引有效性 / 补齐语义向量 / 重提取缺失内容）完成后，**状态栏 📋 图标点亮**并显示结果摘要（如「索引有效性验证完成: 检查 N，恢复 N…」）。点击图标 → 跳到日志页定位到该任务的 `[TASK]` 日志行，并**自动暂停自动滚动**便于查看。未点击前，切页/重启后图标保持点亮（已读状态持久化）。

### 5.2c 批量重新索引

浏览页 Cmd/Ctrl+单击多选或 Shift 区间多选文件后，右键菜单「批量重新索引」一次性提交所有选中文件（替换旧的逐条提交）。
...
3. 分类不准确时：在 Provider 的模型子列表中手动把模型类型改为 embedding / LLM（保存即生效）。
4. 每个 Provider 可「测试」连通性、「刷新」重拉模型列表（不影响手动分类）、「编辑」「删除」（正在使用的 Provider 不可删，需先切换当前模型）。
5. **测试连接**：旧版按钮仍可分别验证当前 Embedding / LLM 模型连通性。

配置后可用（Embedding 与 LLM 可指向不同服务器）：

- **语义搜索**：搜索页点击「✦ 语义」按钮——按"意思"而非仅按"字面"匹配。完整用法（前提、步骤、区别、注意事项）见上文 **2.8 语义搜索** 节。
- **文档摘要**：预览面板点击「✦ AI 摘要」，大模型生成该文档的要点总结（自动缓存，再次打开秒回）。依赖 LLM 模型。
- **跨文件问答**：浏览页多选文件 → 底部「问 AI」输入问题 → 基于选中文件的内容回答。适合"对比多份合同""总结这批材料"。依赖 LLM 模型。

### F. AI 聊天（多轮对话 + 检索范围控制）

AI 聊天页（左侧导航栏聊天入口）提供多轮对话式文档检索，支持**精确控制检索范围**：

**检索范围控制**（输入时，范围不依赖 LLM，LLM 不碰范围决定权，不提即全库）：

| 方式 | 操作 | 示例 |
|------|------|------|
| **@ 文件** | 输入 `@` 弹出文件/目录选择器，选中即插入 `@路径` | `@财务/年度报告.md 汇总收入` |
| **@ 目录** | 同上，选择目录或树状目录右键「加入对话」 | `@财务 本季度收入` |
| **@ 目录 子目录** | 树状浏览器右键加入子目录，自动按路径前缀过滤 | 树状浏览器操作 |
| **条件命令** | 输入 `/ext:pdf` `/date:2025-01-01~2025-12-31` —— 作为 AND 条件过滤当前范围，chips 区实时显示 | `/ext:pdf 年度报告` |
| **会话范围（持久）** | 树状浏览器目录后「范围」按钮设当前目录为**会话范围**（多轮追问持续限定），再点清除回全库；或输入 `/范围:全库` / `/范围:目录名` | 树状操作 / `/范围:财务` |
| **树状文件浏览器** | 左侧栏文件树面板，文件左键/目录右键「加入对话」 | 点击操作 |
| **范围条 chips** | 输入框上方实时显示当前 @mention 路径（紫）与 /命令条件（蓝），@ 可 × 删除同步清文本 | 点 × 删除 |
| **严格模式** | 输入框上方「仅依据文档」开关——开启后范围内无命中时明确告知"未找到依据"，不自由发挥 | 点 toggle |
| **专注模式** | 树状浏览器文件右键「专注分析」——会话仅分析该文件（📌 显示，追问持续），× 退出回正常范围 | 文件右键 |

**编号引用模型**：`@文件` 在发送给 LLM 前替换为 `[N]` 编号，材料带编号注入，路径字符串不进 LLM，分句映射精确（如"根据[1]、[2]统计总人数，并根据[3]计算平均身高"）。`@目录` 不编号，范围内检索命中进材料。

**追溯面板**：每条回答下方可折叠「🔍 检索依据」面板，展示每份来源的 BM25 分、语义相似度、RRF 融合分、改写标记/改写后查询、历史保留标记。

**多轮对话**：首问全库检索（或按指定范围），追问自动重检索 + 合并对话中提到的旧文件（上限 15 份，旧来源仅保留对话中明确提到的，硬上限 3 份）。回答流式渲染，支持 Markdown。

**聊天的动态依据**：首问按问题检索文档作为依据（顶部显示「基于 N 份文档」）；**追问**会按新问题**重新检索**，与"对话中提到过"的旧依据合并（淘汰失效文件、纳入新命中文档），依据列表随每轮追问更新。切页/重启后回到进行中的问题会自动续跑。

**可用性提示**：未配置或测试不通过的模型，对应功能自动置灰禁用（搜索页语义按钮、预览摘要按钮、浏览问答条），tooltip 会引导到设置页配置。
