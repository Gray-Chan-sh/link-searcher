...
**空态页面**：尚未搜索时，中间区域显示引导文字「搜索您的文档」和「在上方输入关键词，搜索所有已索引的文件」，同时底部状态栏显示文件总数和索引进度概览。

## 步骤 2：输入关键词搜索

1. 点击顶部的搜索框（或按 `⌘K` 快捷键聚焦），搜索框提示文字为「输入关键词搜索... (⌘K)」
2. 输入关键词，例如 `预算`
3. 按 `Enter` 执行搜索
...
点击「筛选」按钮展开/收拢左侧筛选面板，面板包含两类筛选：

**目录筛选**：
- 列出所有已添加的资料库目录（如 `qa-corpus`、`Link-Searcher-Demo`、`工作日志`）
- 展开目录树，勾选/取消勾选目录
- 只搜索选中的目录及其子目录
- 父目录与子目录联动——勾选父目录自动包含所有子目录

**文件类型筛选**：
- 列出所有检测到的文件扩展名（`.pdf`、`.jpg`、`.docx`、`.png`、`.doc`、`.xlsx`、`.md`、`.txt`、`.xls`、`.ppt`、`.pptx`、`.m4a`、`.zip` 等）
- 勾选扩展名，只搜索勾选格式的文件
- 取消勾选全部，则搜索所有格式
...
- **PDF 标识**：显示「PDF 文件」与「OCR 文本内容」标识
- **图片缩放**：图片文件可缩放查看
...
| 语法 | 示例 | 说明 |
|------|------|------|
| 通配符 | `doc*` | 前缀匹配 — 匹配 document、docx 等 |
...
🔍 **搜索流程**：[查看交互式图表](diagrams/10-search-flow.html)
