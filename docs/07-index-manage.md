...
**统计卡片**：五张统计卡片分别显示：
- **总文件**：资料库中的文件总量（如 `11,671`）
- **已索引**：成功提取文字并索引的文件数（如 `11,666`）
- **待处理**：尚未处理的文件数（如 `0`）
- **已 OCR**：完成 OCR 识别的文件数
- **错误**：处理失败的文件数（如 `5`），点击「详情」可查看错误列表
...
- **「⟳ 重建语义向量」**：清空全部语义向量并用当前嵌入模型重新嵌入（切换嵌入模型后使用；向量维度与模型不一致时页面会给出告警横幅）
...
点击「开始扫描」按钮，执行一次**全量扫描**：

- 重新遍历所有资料库目录的全部文件，对比 mtime 与内容，处理新增和修改的文件
- 扫描过程中进度条与状态栏会显示进度
- 如需中途停止，可点击「取消」
...
👁️ **实时文件监控**：[查看交互式图表](diagrams/11-watcher.html)
