...
| ID | 名称 | 验收人 | 签收方式 |
|---|---|---|---|
| M-01 | 首启向导"是否顺畅" | 人 | 全新数据目录启动→走完向导→跳过/安装→主界面可用，体验流畅无卡顿 |
| M-02 | 错误消息"是否可操作" | 人 | 触发各种错误（扫描失败/OCR 失败/AI 超时）→消息可读、有建议、无裸堆栈 |
| M-03 | 语义开关"是否可发现" | 人 | 搜索页找到 ✦ 按钮，位置合理 |
| M-04 | macOS Apple Vision 默认 | 人 | 设置页→OCR 引擎→Apple Vision 标记为默认/可用 |
| M-05 | Windows 无 cmd 闪现 | 人 | Windows 上触发含 OCR 的扫描→观察无黑窗闪现 |
| M-06 | Ollama Provider 模型分类 | 人 | 设置页添加 Ollama→模型列表拉取→embedding/LLM 分类正确 |
