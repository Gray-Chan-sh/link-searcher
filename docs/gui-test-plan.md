...
> **已实现 101 个可视化 GUI 测试用例**（P0 30 + P1 57 + P2 14），覆盖 9 个页面 + 跨页面流 + 键盘导航 + 备份。
> 全部基于真实 App 窗口 + MCP（`execute_js` JS 驱动），测试期间可正常使用电脑。
> ✅ **2026-09-07 全量验证：101/101 通过**
>
> ⚠️ **权威来源**：本文件是**测试方案/设计稿**（历史）。实际执行方式、完整用例分布与结果以 [`TEST_REPORT.md`](TEST_REPORT.md) 为准；下文统计表已按当前实测同步（101 用例 = P0 30 + P1 57 + P2 14）。
...
**修正**：最终 **101** 个用例（P0 30 + P1 57 + P2 14）全部实现并通过；上表已按实测同步，权威结果见 [`TEST_REPORT.md`](TEST_REPORT.md)。
...
```bash
# 对比 current/ vs baseline/ 每张截图（Python Pillow，test-visual/pixel-diff.py）
# 阈值: 差异 > 0.5% 才报为视觉回归

python3 test-visual/pixel-diff.py baseline/ current/ diff/
# 差异图用红框标出变化区域，嵌入报告
```
...
#### G-03: 首启向导可跳过 [P1]
前置: 全新数据目录（删除 `~/Library/Application Support/link-searcher` 后启动）
| 步骤 | 操作 | JS 断言 | 预期 |
|------|------|---------|------|
...
|----|------|---------|-------------|--------------------------|
| M-01 | 首启向导"是否顺畅" | 全新数据目录（备份后清空 `~/Library/Application Support/link-searcher`） | ① 启动 App ② 观察向导出现 ③ 点「跳过」④ 验证主界面可用 ⑤ 重新启动观察安装流程 | 向导文案清晰无错位、跳过即时生效、主界面无卡顿、可成功添加目录 |
...
| 4. P0 用例 | 30 个 P0 脚本编写 | ✅ 完成 |
| 5. P1 用例 | 57 个 P1 脚本编写 | ✅ 完成 |
| 6. 全量验证 | 101/101 用例通过 | ✅ 完成 |
