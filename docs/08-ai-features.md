...
**范围条**：输入框上方显示当前会话已累计的范围条目（📄 文件 / 📁 目录），逐条可点击 × 删除；无范围时显示「未限制（全库）」。

**跨轮累计**：本轮的 @引用（chips）发送后自动并入会话范围，下一轮继续生效，直到手动删除。

**父目录吞并子路径**：当范围同时包含 `A` 和 `A/B` 时，自动合并为 `A`（父目录已覆盖其全部内容），避免检索被无谓收窄。

**范围快照**：每一轮发送时的完整范围会被记录，导出会话时可逐轮追溯该轮实际检索了哪些路径。
