...
**文件树浏览器**：会话列表下方有「文件树 ▸」按钮，点击展开后显示所有资料库目录。每个目录旁有「范围」按钮，点击可将该目录设为当前会话的检索范围（再次点击取消）。目录下的文件和子文件夹可点击展开，文件前有 📄 图标。
...
AI 聊天支持精确控制检索范围。输入框下方会显示当前已生效的检索条件 chips：
...
| 会话范围 | 左侧文件树目录后点「范围」按钮 | 设置当前目录为会话范围 |
| 严格模式 | 开启「仅依据文档」开关 | 范围内无命中时拒绝回答 |
| 专注模式 | 文件右键「专注分析」 | 仅分析该文件 |

输入框下方 chips 区域实时显示已生效的 @mention（紫色，📄/📁 前缀）和条件命令（蓝色，`/ext:` `/date:` 前缀），可点击 × 删除。
...
- **仅依据文档**：输入框下方开关，开启后范围内无命中时拒绝回答（绿色激活态）
