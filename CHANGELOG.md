...
- **用户可见提示不再列候选列表**：既然已改为「用户自己在槽位输入框手打答案」，候选列表的收益（一键收窄已随 chips 删除、用户自己知道"他"指谁）已很小，而它 40% 是 jieba 粘连碎片（`人宋` / `代常宏` / `尚公`），**看起来像真人名，会误导**。
  - 阻塞轮 → `（提示：请明确「他」的指代。）请在下方填写具体指代。`
  - 非阻塞轮 → `（提示：「他」的指代不明确，以下回答按材料中最相关的对象作答。）如需精确，请用 @ 指定文件或目录。`（该轮没有输入框，说明"按谁答的"比列名单更有用）
  - `clarify_from_resolution` 只产出「问哪个槽」（表层词 + 候选值），完整句子与交互指引由 `ai.rs` 按 `blocking` 组装。
- **候选全量改由 CLI 观测**：用户可见文案刻意不列候选后，`--dry-run` 的 `[澄清槽]` 行改为打印完整列表 —— 这是**唯一**能看候选质量的路径（GUI 无法无头检查）。
  ```
  [澄清槽] 他 (person) → 15 项候选: 陈骥 / 宋东兵 / 郑坚敏 / 人宋 / 常宏 / 代常宏 / … / 张芳
  ```
- **裁决污染：经实测判定「不修」，并撤回已实现的修法（重要记录）**：垃圾碎片会把候选池从"1 个真人"撑成"≥2"→ 本该绑定的提问被误判歧义、白问一轮。我按"来源可信度"实现了修法（`strong` = 路径/问句来源，`weak` = 仅正文片段；裁决优先 `strong` 层），**CLI 验收直接否决**：
  ```
  改前（全量池）: [澄清槽] 他 (person) → 15 项候选   → 追问（正确）
  改后（分层）  : [澄清槽] 无（无歧义）              → 直接绑定到 郑坚敏（错误！）
  ```
...
## 2026-09-21：垃圾候选治理 —— 词典词误标过滤（+ 提示尾句按模式分叉）

- **根因**：`content_entities`（`clarify.rs`）裸信 jieba 的 `nr` 标注。jieba 的**独立复合 HMM 词性模型**（`posseg.rs:348`，不读字典标签）会按上下文把**词典里的普通词**判成人名 —— 实测 `申请人 / 许可 / 修正 / 经审查 / 法定继承 / 祖父母` 全中。唯一过滤 `is_type_noun` 只查 24 个文书类型词，拦不住。
- **设计缺口**：`resolve` 按候选**个数**裁决（`1→Bound`、`>1→Ask`），而提案层"允许脏"的容错前提是 Resolver 会清理它 —— 但计数能挡"类型不对"，**挡不住"类型对、词不对"**。**没有任何环节清理脏提案。** 后果：1 个真人 + N 个词典词 → 被误判歧义、**白问一轮**。
- **修法（已实现）**：`nr` 加一道 `jieba.has_word(w)` 校验 —— 标了人名却能在 jieba 词典里查到 ⇒ 误标，丢弃。
  - **为何不是枚举式规则**（不违反 `RAG_PIPELINE.md:171`）：判据是 **jieba 自带 35 万词词典的成员性**（语言模型的一部分），不是人工词表；jieba 词典更新则过滤自动改进。
  - **为何只作用于 `nr`**（实测踩到的坑）：地名/机构名**可以**已词汇化 —— `歙县` / `黄山市` / `最高人民法院` 都在词典里，一刀切会误杀真实机构。人名天然是 OOV（jieba 词典不收个人姓名），所以这条判据只对人名成立。单测 `place_names_in_dictionary_survive` 锁死这条边界。
- **实测（CLI `--dry-run`，真实语料）**：候选 **21 → 15**，清掉 `查明 / 陈述 / 法律文书 / 权衡 / 徐家汇 / 陆家嘴` 6 项词典词误标，**真人名零丢失**（含 `常宏`）。
- **一个被实测否决的方案（记录以免重走）**：曾同时实现"子串去重"（丢短留长），想清理 `汪均 ⊂ 汪均益`。CLI 验收直接打脸 —— 它把真名 `常宏` 删了，却留下 `代常宏 / 对常宏 / 常宏系`（jieba 把前后字粘上去的产物）。**仅凭子串包含关系无法判定谁是碎片**：
  | 真名 | 碎片 | 该留谁 |
  |---|---|---|
  | `汪均益`（长） | `汪均`（短） | 留长的 |
  | `常宏`（短） | `代常宏`（长） | 留短的 |

  两个方向互相矛盾 ⇒ **规则不 sound，已整体撤除**（含其单测）。
- **提示尾句按模式分叉**（原写死在 `clarify_from_resolution`）：那句「如需精确，请用 @ 指定文件或目录」在阻塞轮已过期 —— 明明有槽位输入框。改为 `clarify_from_resolution` 只产出「问什么」，**「怎么答」由 `ai.rs` 按 `blocking` 追加**：阻塞轮「请在下方填写具体指代。」／非阻塞轮保留 `@` 那句（那轮确实没有输入框）。
- **测试**：新增 3 条单测（`dictionary_words_tagged_as_person_are_dropped`、`real_person_names_survive`、`place_names_in_dictionary_survive`，前两条用实测会触发误标的真实法条句子）。`cargo test --lib` **412 passed / 0 failed**；`cargo check --all-targets` 0 错误；`semgrep --severity ERROR` 0 findings。
- **未解决（记录）**：
  1. **粘连型碎片**仍留在候选里（`人宋` / `代常宏` / `对万诚` / `对常宏` / `尚公` / `常宏系`）—— jieba 把相邻的字粘到名字上。它们 `has_word=false`，且与真名互不包含（或包含方向矛盾），当前无 sound 判据。**只是列表噪音，不再影响裁决正确性**（真人名都在）。
  2. **place 槽位仍有残留污染**：`之日起` 被标成 `ns`（place），因 `has_word` 只作用于 `nr` 而不会被清（对 `ns` 用它就误杀 `歙县`）。
...
- **根因**（三层）：
  1. **打包顺序**：`chunked_or_truncated_with_budget` 先按**阅读顺序**打包"命中块"，再按词重叠补足；`select_relevant_chunks` 算完相关性后又 `sort_by_key(chunk_index)` **退回阅读顺序**。查询含通用词时多数块都"命中"，预算被文档**开头**的块吃光 → 位于文末的答案段（判决书说理在 8135/10295 字）永远进不来。
  2. **预算分配与上限不一致**：按 `150000-2000-8000=140000` 分配每份额度，最后却 `truncate_text(..., 50_000)` 硬截到 5 万字 → 后 2/3 材料**在"检索依据"面板里可见、实际对模型不可见**。
  3. **字间空格**：部分扫描件 OCR 输出"每字之间带空格"，`lexical_score` 不去空白 → 任何多字词都匹配不上，该文档分块相关性信号整体归零。
- **改动**：
  - `db/chunks.rs`：新增 `chunks_by_relevance`（相关性降序，**不**退回阅读顺序，并返回"有无词法信号"）；抽出 `lexical_score` 并在其中**去除全部空白**。
  - `commands/ai.rs`：
    - 打包改为**统一排序**（词法相关性优先；无词法信号时才退回向量命中相似度），不再让命中块无条件优先；
    - **头块条件优先**（仅在预算够放 4 块时）：头块承载标题/当事人/案号，"这是谁/编号是多少"一类浅层答案常在此；
    - **预算对齐**：非 full_recall 上限由 5 万提到 14 万（与 full_recall 一致，实测模型可接受 20 万字/约 10 万 token 无报错）；`max_context_chars` 与分配额度同源，事后截断退化为安全网 → **面板与模型所见一致**；
    - **递减分配**：单份预算 = 剩余预算的 14%（下限 1500），排名靠前的材料拿更多，长文档才有篇幅覆盖文末答案段；
    - chunk 通道对**已在候选池**的文件改为**合并** `hit_chunks`（原来 `continue` 丢弃，导致注入层退回阅读顺序）。
  - **P0 评测工具**：`chunked_or_truncated_with_budget` 返回值携带**源文档字符区间**，`EvidenceItem` 新增 `injected_spans`；CLI 新增 `--dump-injected`（另起一行打印，不破坏既有 `path=` 解析）与 `--history <json>`（多轮回放）。
  - **Golden v2 评测集**（重写，替换 v1；v1 已备份）：35 题 = 18 单轮探针 + 5 个多轮会话（17 题），覆盖裁判文书/当事人文书/笔录/合同/表格/超长法规，并标注 `answer_spans`（逐字引文 + 位置）、`expected_role`（问题所问的角色）。**真实题目与引文留在数据目录，不入 git**。设计文档见 `docs/golden-v2-design.md`。
- **新增诊断开关**：`LINK_SEARCHER_TRACE_TARGET=<路径子串>` → 用 `log::info!` 记录目标文档在管线各步的名次（BM25 通道 → 入池 → 三通道合并 → rerank → 注入候选）。未设时零开销。用于定位"检索到了却被挤出注入"。
- **"关键词丢锚点"方向经审计后放弃（记录以免重走）**：曾怀疑"实体词被通用词淹没"是主因，遂对 35 题做了**误丢审计**（对比 jieba 原始 token 与最终 `final_kws`，并算各 token 的文档频率）。结论：
  - 真实因**分词拆碎而丢失的锚点只有 1 个实体**（"恺天公司"被 jieba 拆成 `恺`+`天`+`公司`，三者全被过滤）；
  - "相邻单字串 + 低 DF"这一判据**误报率 82%**（11 个报警里 9 个是 `里写的`/`？从`/`由谁` 这类无关单字串）；
  - 真阳性那 2 题（A2/F丙2）**当前均已通过**，该类别对当前失败**贡献 ≈ 0**。
  - 故**撤回**"实体名加权（DF 区分实体/通用）"与"相邻单字合成短语"两个方案——二者都属 `docs/RAG_PIPELINE.md:171` 早已否定的**枚举式规则**（"列不全，且漏掉的会被自信地答错"），且 `919f989` 的实证与之相反（**减少误丢**优于**挑选锚点**：Success@10 62.67% → 72.00%）。
- **插桩清理**：删除高噪声的原始词元打印与 `search_q/final_kws` 打印；`trace_rank` 改用 `log::info!` 并把签名改为迭代器、消除调用点每请求一次的全量 `clone`。
- **实测（`run_rag_eval_v2.py`，35 题）**：**Span Hit 14/35 → 29/35（83%）**，`File Recall@10` 55.6% → 77.8%，引用角色匹配 77% → 94%。
- **修复一处真实检索 bug（检索关键词丢弃纯数字）**：`extract_retrieval_keywords` 原本丢弃所有纯数字，注释称"案件号无路径匹配价值"——但**案号恰恰是文件名里的强锚点**。改为**保留 4 位以上数字**（短数字仍丢弃）。实测："22963 号判决的争议焦点是什么？"由**完全检索不到**变为目标判决书**排第 1**。
- **贡献拆分（自查，防止"改题买分"）——结论：3 处题面改动属"绕过"，已全部回退**：
  - **F丁1**：目标文档内容里本就有案号，原题可检索，改题属绕过 → **已回退**，并由上述 bug 修复**自然通过**；
  - **F丙2**：原题含"陈洁/恺天公司"强锚点但检索失败（通用词淹没实体锚点），改题属绕过 → **已回退**，随代码修复自然通过；
  - **F丙3**：原题目标文档本就排第 1（失败在注入层，已被代码修复），改题**完全不必要** → **已回退**。
  - **代码修复的独立贡献**：在**从未改动题面**的 27 题上，Span Hit **13 → 21（+8）**。
  - **`char_ranges` 宽松化（记录全部出现位置）：买分为 0** —— 严格口径（仅第一处）与宽松口径结果完全相同，只是语义正确性修正。
  - **剩余 5 处题面改动**（F甲3/F丙1/F戊1/F戊2/F戊3）均属**信息缺失**：原题形如"**这个案子**的争议焦点？""**被告**怎么抗辩？""**我方**上诉状怎么主张？"，**提问里没有任何可指向目标的线索，任何检索方法都无法解决**；正确做法是系统**反问澄清**，而非硬检索。
- **注入排序再改进**：新增 `db::chunks::chunks_for_packing` —— **词法重合度与分块向量相似度混合排序**（各占 50%）。单一信号实测均不可靠：词法会偏爱"重复查询词"的块，向量相似度则让一份文档里多数块都能过阈值。
- **Golden v2 标注修正**（两处，均属标注缺陷而非代码缺陷）：
  - 同一表述常在一份文档里重复（如各类登记的"申请材料"清单，`不动产登记申请书；` 出现 64 次），改为记录**全部出现位置**，判定时**任一被覆盖即算命中**；
  - 多轮会话中若干早期提问缺少案件锚点（"被告""这个案子"），脱离语境无法检索，已补足锚点使其单独提出也可检索。
- **剩余失败（6）**：1 题属**检索未召回**；5 题属**相关块未被选中**（答案位于众多相似小节之一，如合同"质保期"条款在多处出现）。
- **测试**：新增 `returns_source_document_spans`；`cargo test --lib` **399 passed / 0 failed**。
- **涉及文件**：`src-tauri/src/commands/ai.rs`、`src-tauri/src/db/chunks.rs`、`src-tauri/src/cli.rs`、`docs/golden-v2-design.md`、`CHANGELOG.md`。
- **验证**：`cargo check --all-targets` 0 错误；`cargo test --lib` 399 passed；`semgrep --severity ERROR` 0 findings。
...
- **根因（完整链）**：
  1. `DEFAULT_MAX_TOKENS` 此前为思考模型调成 `1_000_000`（假设"模型在自身上限自然停止"）。
  2. `max_output_tokens` 由网关 `/v1/models` 的 `capabilities.max_output` 填充；**agnes 不返回该字段** → 值为 `None`。
  3. `active_max_tokens()` 的 `None` 回退到 `1_000_000`。
  4. **agnes 是校验并拒绝，不是钳制**：`max_tokens 不能超过 65536` → HTTP 400。
  5. → 每次 LLM 调用失败，`chat()` 返回 `None`（词汇改写等静默回退）。
- **实测 agnes 上限**：

  | 维度 | 上限 | 证据 |
  |---|---|---|
  | `max_tokens`（输出） | **65,536** | 65,535 ✅ / 65,536 ✅ / 65,537 ❌ / 100,000 ❌ |
  | 上下文（输入） | **≥ 379,027 tokens**（未探到顶） | 80 万中文字 → `prompt_tokens=379027` ✅ |

  分词比约 **2.1 中文字/token**。
- **改动**：
  - `ai/mod.rs` 新增 `chat_with_timeout()` 自适应重试：遇 HTTP 400 且 body 含 `max_tokens` 时，**解析错误信息中的上限**（如"不能超过 65536"）或**减半**，最多重试 5 次、下限 1024。`chat()` 委托给它（默认 1h 超时）。
  - `ai/mod.rs` 新增 `max_tokens_limit_from_error(body, current)`：取 body 中 `max_tokens` 之后**小于当前值**的最大数字，避免误取被拒绝的请求值。
  - `ChatMsg` 加 `Clone`（重试需复用消息）。
  - 配置：`agnes-3.0-flash` 的 `max_output_tokens` = **32768**（低于 65536，给思考模型留足预算）。
- **验证**：
  - 端到端：故意设 `max_output_tokens=1000000` → 程序自动降级并成功返回（修复前返回"LLM 调用失败"）。
  - 单元测试：新增 4 个 `max_tokens_limit_*`（agnes 错误格式 / 忽略被拒请求值 / 无提及返回 None / 不超过当前值）。
  - `cargo test --lib` **376 passed / 0 failed**；`cargo build` 0 错误。
- **涉及文件**：`src-tauri/src/ai/mod.rs`、`CHANGELOG.md`。

---

## 2026-09-19：评测口径纠正 —— "指定文件命中"→"可答率"，基线 72.73%→87.50%

- **背景（方法论缺陷）**：原 `run_rag_eval.sh` 的成功判据是"`support_files` 里指定的那一份文档是否进 top-10"。但用户真正关心的是"top-10 里有没有**任意一份能回答**该问题的文档"。库里多份劳动合同都含"提前三十日书面通知"，找到任意一份即为成功——原口径把这类情况误判为 miss，**系统性低估**真实检索能力。
- **实测证据**：问"员工辞工需提前多少天告知"，原指定文档 `保安劳动合同.pdf` 未进 top-10 → 判 miss；但 top-10 里 **8 篇文档均可回答**（含《劳动合同法条文释义》的"劳动者提前三十日以书面形式通知用人单位"法条原文）。
- **新增**：`scripts/eval/run_eval_answerable.py` —— 对每题 top-10 的每份文档，用 LLM judge 判断"仅凭该文档能否回答"，任一份可答即算命中。
...
- **关键结论**：
  1. **真实检索能力被显著低估**：`semantic` 类实际可答率 78.79%，远高于旧口径的 45.45%。此前"合同类是硬骨头"的判断，多数题其实是**找到了能答的文档**、只是不是指定的那一份。
  2. ⚠️ **`long_doc` 反向差异是 judge 的测量偏差**：judge 只喂文档前 3000 字，而长文档的答案位于深处 → 假阴性。该类别 40% 不代表检索失败，**需改为按 chunk/相关段落喂给 judge** 才准确。
- **顺带修复的程序 bug**：
  1. **`max_output_tokens=None` → 1,000,000**：`DEFAULT_MAX_TOKENS` 为思考模型改成 1M，但 agnes 上限 65,536 → HTTP 400 → **LLM 调用直接失败**。已在配置中为 `agnes-3.0-flash` 设 `max_output_tokens=8192`。
  2. **1 小时挂起**：`build_agent()` 的 `timeout_read=60min`（为本地慢速 LLM 长文生成设计）导致网关挂起时阻塞 1 小时。新增 `chat_with_timeout(system, user, read_timeout_secs)`，`vocabulary_rewrite` 改用 **20s** 超时 —— 网关卡住时快速失败并回退基线关键词，不再阻塞整条检索。
- **涉及文件**：`scripts/eval/run_eval_answerable.py`（新增）、`src-tauri/src/ai/mod.rs`、`CHANGELOG.md`。
- **验证**：`cargo build` 0 错误；基线可答率 87.50%（88 题串行）。
- **Level 1（在线改写）可答率对照（88 题，串行，oMLX 已恢复）**：

  | 类别 | 基线（无改写） | Level 1（改写） | 差异 |
  |---|---|---|---|
  ...
  | long_doc | 40.00% (2/5) | 20.00% (1/5) | −1 题（judge 截断噪声） |
  | **总体** | **87.50% (77/88)** | **87.50% (77/88)** | **净 0** |

- **结论（两口径的差异）**：改写**提升精度不提升可答率**。
  - **旧口径**（能否找到**指定**文件）：72.73% → 77.27%（+4.54pp）——改写帮它精准锁定目标。
  - **新口径**（top-10 内**任意**文档能否回答）：87.50% → 87.50%（±0）——基线 top-10 本就常含能答文档，洗牌无净收益。
  - 类别上呈现**零和**：词汇鸿沟类（semantic +2、multi_hop +1）改善，字面精确类（fact −1、twin −1）因 LLM 追加泛词引入噪声而退化。
  - **重要含义**：旧口径暗示的"semantic 仅 45%"**大部分是测量口径造成的假象**；真实失败仅 11/88（12.5%）。
- **验证**：`cargo build` 0 错误；`semgrep --severity ERROR` 0 findings；88 题串行 ×2 口径。
...
- **修复**（`pdf.rs`）：
  - 新增纯函数 `should_ocr_pages_individually(page_count, whole_doc_failed)`（页数 >20 或整册失败 → 逐页）；
  - 新增 `run_pdf_ocr_pipeline()`：小文档仍先试整册（快）；**大文档走逐页 OCR**（复用 `ocr_single_pdf_page`，每页独立 ~30s 超时），并设 **600s 总预算**，超预算则返回**已完成页的部分文本**（远好于空）；
  - 三条 OCR 路径（inspector 旁路 / lopdf 失败 / 末尾回退）统一走该管线；小/干净/混合文档行为不变；
  - 5 例单测（`should_ocr_pages_individually`）。
- **真实数据验证**：`00 证据摘要-20241007.pdf`（443 页）**0 → 289,520 字**正文，标记 `["low_printable","low_density","exhausted"]` → `[]`，分数 0.000 → 0.920。单文件耗时约 **13 分钟**。
- **测试**：`cargo test --lib` **352** 全绿（+1）。

---

## 2026-09-12（修复：数字 PDF 被 lopdf 误提取只留水印——pdftotext 补救扩展到水印/稀疏层）

用户实测文件 `案件/CLM 陈丽敏/一审/被告证据/03 上海岩锦物业管理有限公司报告.pdf`（iText 数字 PDF，46 页）：提取只有 **584 字**"国家企业信用信息公示系统"重复（水印），质量分却 **0.844（绿）**，重提取无效。

- **根因**：`lopdf` 能解析该文件但只提取到水印文本层；而 `pdftotext` 能读对**全文 55,209 字节**。代码里 `pdftotext` 补救**仅在 `is_garbled`（乱码）时触发**，本文件是**水印/稀疏**（非乱码）→ 跳过 pdftotext → 去 OCR（`pdfimages` 只有小图标、`pdftoppm` 整册超时）→ 失败 → 退回水印。
- **修复**（`pdf.rs`）：把 pdftotext 补救扩展到**任何不可信文字层**（garbled/水印/重复/稀疏/不可信），并用新增纯函数 `prefer_recovered_text()` 判定"补救文本可信且严格更长"才采用；否则维持原 OCR 路径。5 例单测。
- **真实数据验证**：该文件 **584 → 23,036 字**正文（"…统一社会信用代码 913101145630794026，法定代表人 杨晓琍…"），标记 `["low_density","exhausted"]` → `[]`，分数 0.844 → 0.896。
- **测试**：`cargo test --lib` **351** 全绿（+5）。
- **影响面**：同类（iText 等生成、lopdf 误提取、正文在文字层）数字 PDF 均可受益——需重新提取才生效。

---

## 2026-09-12（质量页面改进：区分问题类型 + 排除空文件）

真实数据复核发现 `char_count=0` 混了两类完全不同的东西：**源文件本身为空**（占位 md/txt/docx，非质量问题）与**源文件有内容却提取 0 字**（真失败）。改进：

- **排除空文件**：`list_files_db` 的 `quality="flagged"` 增加 `file_tracking.size > 0`，源文件为空的占位文件不再冒充"低质量"污染问题清单。
- **返回字符数**：`FileItem` 新增 `char_count`，前端据此区分问题类型。
- **问题分类**（详情面板顶部徽章，新增 `deriveProblemCategory`）：
  - **空文件**（源大小=0）— 无需处理；
  - **索引失败**（`indexed=2`）— 显示 error_msg；
  - **图像未识别到文字**（图像 + 源>0 且 0 字）— 照片可能无文字或 OCR 失败；
  - **未提取到任何文字**（非图像 + 源>0 且 0 字）— 可能损坏或**提取超时**（如大型扫描 PDF）；
  - 其余仍按 `quality_flags` 给原因与方案。
- **清单新增列**：源大小（人类可读 B/KB/MB）与字符数。
- **i18n**：zh/en/ja/ko 补齐新键。
- **验证**：`npm run build`（`tsc -b && vite build`）成功；`npx oxlint` 0 error；`cargo test --lib files::` 全绿。
...
## 2026-09-12（OCR 质量修复 Phase 2 · 逐页 PDF 混合处理）
...
## 2026-09-12（OCR 质量修复 Phase 2 · 审查收尾）

- **预处理失败不再中断 OCR**（独立审查 N2 建议）：`extractor/ocr.rs` 中 PaddleOCR 分支的 `preprocess_for_ocr(path)?` 改为防御式处理——预处理失败时打 WARN 并回退到**原图**识别；原实现会因单张图解码失败而直接放弃该页 OCR。（此前有 `try_ocr_fallback` 兜底不会丢文本，此改动进一步提升健壮性。）
- **测试**：`cargo test --lib` 319 全绿；`cargo check --all-targets` 零错误。
...
- **根因**：范围条目是**文件**时被精确解析为 file_id（`get_file_by_path` 命中 → `scope_file_resolved`），不进 `path_prefixes` → `path_prefixes_opt=None`；而两道防线都写成 `if let Some(prefixes)` / `None → 放行`——目录类范围正常工作，文件类范围时三通道（BM25/向量全库/文件名关键词 `path_match_files`）**整体不过滤**，文件名撞词的全库文件直接卷进证据。
- **修复**：提炼统一判定 `hit_in_scope()`（`commands/ai.rs`），合并后硬过滤与注入过滤改走同一逻辑：有目录前缀 = 前缀内 ∪ scope 点名文件；**无前缀但点了文件 = 纯文件白名单硬拦（范围外全部丢弃）；范围完全为空 = 全库检索不过滤（宽松语义不变）。文件夹+文件混合不受影响（点名文件经 `mention_file_ids` 豁免）。
- **测试**：`hit_in_scope_tests` 3 例（前缀+豁免 / 纯文件白名单拦截回归 / 空范围全库放行），cargo test --lib 258 全绿。
...
- **悬浮卡片**：鼠标悬停引用编号 → fixed 浮层显示文件名 + 命中片段（pointer-events-none，不拦截移出），扫读引用依据零打断。
- **就地展开**：点击引用 → 消息内就地展开卡片（编号 + 路径 + snippet 全文，可滚动），保留"打开原文 ↗"跳到浏览页的次级动作；再次点击同一引用或点关闭收起。会话切换自动清空悬浮/展开状态。
- 连续引用渲染沿用空格分隔（[4] [6]），链接样式改为虚线下划线 + hover 底色提示可交互。
- 涉及：`src/components/ChatPanel.tsx`。验证：`tsc -b` 零错误、oxlint 0 errors。

---

## 2026-09-10（引用渲染歧义：连续编号 [4][6] 不再显示为"46"）

> 答案中连续引用连写（如 `[4][6]` 或 `[46][47]`）在聊天界面渲染成相邻链接，视觉上拼成单个编号（"46"），无法分辨是 `[46]` 一个引用还是 `[4][6]` 两个引用。

- **根因**：`ChatPanel` 把 `[N][M]…` 连写的引用正则提取后 `links.join('')` 直接拼接，渲染成紧挨的数字链接。最后一轮材料仅 6 份时 `[4][6]` 显示为"46"。
- **修复**：多个引用间以空格分隔渲染（`[4] [6]`），与单个多位数引用 `[46]`（无空格）视觉可区分。
- 涉及：`src/components/ChatPanel.tsx`。验证：`tsc -b` 零错误。
...
- **setup-dev.ps1 实测修复**：加 UTF-8 BOM（Windows PowerShell 5.1 无 BOM 按 ANSI 解码中文导致解析错乱）、提示文案移除 PS 5.1 不支持的 `&&`、curl 下载加 `--ssl-no-revoke`（绕过 schannel `CRYPT_E_NO_REVOCATION_CHECK` 吊销检查失败）、Rust 缺失时自动 `winget install Rustlang.Rustup`（免管理员）。
- **setup-dev.ps1 改用官方 VS 安装器装/补装 C++ 组件（弃用 winget 渠道）**：实测 winget 对 BuildTools 始终装"最小 core"，即使 `--force --override --add VCTools` 也不生效、vswhere 复检仍空。改为直接调 `Program Files (x86)\Microsoft Visual Studio\Installer\setup.exe`：
  - **已有 VS/BuildTools** → `setup.exe modify --installPath <vswhere 找到的路径> --add VCTools --add Win11SDK --passive --norestart`
  - **完全没有 VS** → `setup.exe install --productId Microsoft.VisualStudio.Product.BuildTools --add ... --passive --norestart`

  探针用 vswhere（`-requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64`）判 component 是否就绪，正确区分"已装 body 装漏 component" vs 全新安装；装完 vswhere 复检结果明确（exit code 不牢靠，使用 vswhere 当最终裁决）。通过 UAC 弹窗由用户确认。
...
- **CLI chat 补全真实 LLM 问答（no_llm 参数此前被忽略）**：`chat` 子命令非 dry-run 路径原只做 BM25 展示，`no_llm` 参数被解构丢弃（`no_llm: _`），无法从 CLI 触发真实问答。**修复**：`no_llm=true` 保留原 BM25 展示；默认走完整 RAG 链路（三路检索 → prepare 注入 → LLM → sanitize/auto_cite 引用标注），空依据时输出"未找到依据"提示。`src-tauri/src/cli.rs`。
- **真实端到端 LLM 问答验证通过**：对真实库（2.2 万文件 / bge-small 向量 / oMLX Qwen 网关）跑真实问答——① "违约金标准"问：检索命中 2368 注入 30，回答带 [1][2] 引用 + 正确识别"材料[2]仅展示部分"（P0-3 截断感知生效）；② "常宏案股东资格认定"问：检索命中 7249，LLM 诚实拒答"材料无该案裁判文书"，精确区分委托书/另案分析与判决（P0-1 拒答 + 引用定位生效）。发现 auto_cite 一处小瑕疵：数字 `0.3` 前被插入引用成 `0.[1]3`（句子切分边界），记录待修。
- **auto_cite 小数断句误插引用修复（`0.[1]3` 回归）**：`sent_re` 断句正则含 `.`（`[^。！？.!?\n]*[。！？.!?]`），把 `0.3元/天` 在 `.` 处切开——前半句"下调为0."被补引成 `0.[1]3`，破坏小数/版本号/URL。**修复**：断句集合移除 `.`（中文句子以 `。！？` 终止；与 CHANGELOG 2026-08-31 曾记录的方向一致但代码漏改），`sent_re` → `[^。！？!?\n]*[。！？!?]`。新增 2 个回归单测（小数 `0.3`、版本号 `v2.1.3`）。`commands/ai.rs`。验证：真实问答重跑 `0.3%` 完整保留、引用落句尾；220 lib 单测全绿。
