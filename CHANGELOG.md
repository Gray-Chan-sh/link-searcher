...
- **setup-dev.ps1 自动安装 VS Build Tools**：MSVC 是 Windows 上 `cargo build` 的硬依赖（cc-rs 经 vswhere 定位，`cl` 不在 PATH 也能编译）。脚本用 vswhere 检测 VC++ 工具链（`Microsoft.VisualStudio.Component.VC.Tools.x86.x64`）是否真正可用；**区分两种缺失**：Build Tools 完全未装 → 全新安装（带 C++ 桌面工作负载 + Windows 11 SDK）；已装本体但缺 C++ 组件（winget 默认只装最小 core，vswhere 仍为空——实测踩坑）→ 复用现有安装增量补装组件。装完 vswhere 复检并明确提示结果。
