...
- **setup-dev.ps1 自动安装 VS Build Tools**：MSVC 是 Windows 上 `cargo build` 的硬依赖（cc-rs 经 vswhere 定位，`cl` 不在 PATH 也能编译），此前脚本只提示不装导致用户 `cargo build` 卡在 `link.exe not found`。现在用 vswhere 检测 VC++ 工具链（`Microsoft.VisualStudio.Component.VC.Tools.x86.x64`）是否安装，缺失即自动 `winget install Microsoft.VisualStudio.2022.BuildTools`（带 C++ 桌面工作负载 + Windows 11 SDK），UAC 弹窗由用户确认；装完重开终端即可，不再要求 `cl` 在 PATH。
