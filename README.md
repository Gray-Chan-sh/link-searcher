...
| 功能 | 说明 |
|------|------|
| **语义搜索** | 搜索页「✦ 语义」开关——向量 embedding + BM25 结果 RRF 融合，搜"意思"而非仅"字面" |
| **文档摘要** | 预览面板「✦ AI 摘要」按钮，LLM 生成摘要并缓存 |
| **跨文件问答** | 浏览页多选文件后输入问题，AI 基于选中文件内容回答（RAG） |
| **AI 聊天** | 多轮对话式文档检索——首问自动检索、追问换范围；回答带「检索依据」溯源面板（BM25/语义/RRF 分数 + 改写标记） |
| **检索范围控制** | 输入 `@` 弹出文件/目录选择器、左侧树状文件浏览器/右键「加入对话」/目录一键「设为会话范围」、`@文件` 编号引用 `[N]` 进材料、`@目录` 范围内检索、`/ext:` `/date:` `/范围:` 条件命令与会话范围、范围条 chips 实时显示可删除 |
